...
if ($lastMatch !== null) {
    $lastMatchTime = $lastMatch['matchTime'];
    $lastMatchLeagueDiv = null;
    foreach ($leagueItemDivs as $div) {
        if (strpos($div->textContent, $lastMatchTime) !== false) {
            $lastMatchLeagueDiv = $div;
            break;
        }
    }

    if ($lastMatchLeagueDiv !== null) {
        $h5Element = $xpath->query('.//h5[contains(@class, "color-white")]', $lastMatchLeagueDiv)->item(0);
        if ($h5Element !== null) {
            $h5Text = normalizeText($h5Element->textContent);
            $lastMatch['kolejka'] = $h5Text; 
        }
    }

    echo json_encode($lastMatch, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
} else {
